package com.wangj.baselibrary.view.togglebutton;
...
import com.wangj.baselibrary.facebook.rebound.SimpleSpringListener;
import com.wangj.baselibrary.facebook.rebound.Spring;
import com.wangj.baselibrary.facebook.rebound.SpringConfig;
import com.wangj.baselibrary.facebook.rebound.SpringSystem;
import com.wangj.baselibrary.facebook.rebound.SpringUtil;
...
public class ToggleButton extends View {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		centerY = radius;
		startX = radius; // + borderWidth;
		endX = width - radius; // - borderWidth;
...
//		canvas.drawRoundRect(rect, spotR, spotR, paint);
 		...
 		rect.set(2, 0, getWidth() - 2, getHeight());
		paint.setColor(borderColor);
		canvas.drawRoundRect(rect, radius, radius, paint);

		if(offLineWidth > 0){
			final float cy = offLineWidth * 0.5f;
			rect.set(spotX - cy + 2, centerY - cy, endX + cy - 2, centerY + cy);
			paint.setColor(offColor);
			canvas.drawRoundRect(rect, cy, cy, paint);
		}

		rect.set(spotX - 1 - radius + 2, centerY - radius, spotX + 1.1f + radius - 2, centerY + radius);
		paint.setColor(borderColor);
		canvas.drawRoundRect(rect, radius, radius, paint);

		final float spotR = spotSize * 0.5f;
		rect.set(spotX - spotR + 2, centerY - spotR, spotX + spotR - 2, centerY + spotR);
		paint.setColor(spotColor);
		canvas.drawRoundRect(rect, spotR, spotR, paint);
